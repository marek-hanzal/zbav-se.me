...
describe("flagToggleFx", () => {
	it("toggle on: creates flag record and listing_event", async () => {
		...
		const { user: seller } = await api.signUpEmail({
			body: { email: "seller@flag-toggle-on.cz", name: "Seller", password: "12345678" },
		});
		const { user: buyer } = await api.signUpEmail({
			body: { email: "buyer@flag-toggle-on.cz", name: "Buyer", password: "12345678" },
		});
		...
		// Flag does NOT create an inbox for seller (unlike favourite)
		...
		expect(sellerInbox).toBeUndefined();
	});
	...
	it("toggle off: deletes flag record and creates unflag event", async () => {
		...
		const { user: seller } = await api.signUpEmail({
			body: { email: "seller@flag-toggle-off.cz", name: "Seller", password: "12345678" },
		});
		const { user: buyer } = await api.signUpEmail({
			body: { email: "buyer@flag-toggle-off.cz", name: "Buyer", password: "12345678" },
		});
	...
	it("invalid: seller cannot flag own listing", async () => {
		...
		const { user: seller } = await api.signUpEmail({
			body: { email: "seller@flag-own.cz", name: "Seller", password: "12345678" },
		});
	...
	it("invalid: cannot flag an already-flagged listing (conflict on insert)", async () => {
		...
		// First flag succeeds
		await Effect.gen(function* () {
			yield* flagToggleFx({ userId: buyer.id, listingId: listing.id, toggle: true });
		}).pipe(withRuntimeFx(database), Effect.runPromise);
