...
describe("userEventBuyerInfoFx", () => {
	it("Mixed behavior - combo of good and bad buyer behaviors", async () => {
		...
		const base = DateTime.now().minus({ days: 40 });

		// tx-1: good (open -> message -> success)
		const t1Create = base;
		const t1Open = t1Create.plus({ hours: 1 });
		const t1Msg = t1Open.plus({ minutes: 10 });
		const t1Success = t1Msg.plus({ days: 1 });

		// tx-2: closer (open allowed, buyer closes without other interaction)
		const t2Create = base.plus({ days: 2 });
		const t2Open = t2Create.plus({ minutes: 30 });
		const t2Close = t2Open.plus({ minutes: 5 });

		// tx-3: terminal by seller (open -> seller reject)
		const t3Create = base.plus({ days: 4 });
		const t3Open = t3Create.plus({ minutes: 10 });
		const t3Reject = t3Open.plus({ hours: 1 });

		// tx-4: expired (open -> expired, no buyer action)
		const t4Create = base.plus({ days: 6 });
		const t4Open = t4Create.plus({ hours: 2 });
		const t4Expired = t4Open.plus({ days: 3 });

		// tx-5: good (open -> message -> success)
		const t5Create = base.plus({ days: 8 });
		const t5Open = t5Create.plus({ minutes: 15 });
		const t5Msg = t5Open.plus({ minutes: 15 });
		const t5Success = t5Msg.plus({ days: 2 });

		// tx-6: good, explicit close (recent)
		const t6Create = DateTime.now().minus({ days: 2 });
		const t6Open = t6Create.plus({ minutes: 5 });
		const t6Msg = t6Open.plus({ minutes: 5 });
		const t6Close = DateTime.now().minus({ days: 1 });
		...
		const result = await Effect.gen(function* () {
			...
			yield* userEventCreateFx({
				...
				isTerminal: true,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t1Success })));
			...
			yield* userEventCreateFx({
				...
				isTerminal: true,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t3Reject })));
			...
			yield* userEventCreateFx({
				...
				event: "transaction.create",
				isTerminal: false,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t4Create })));
			...
			yield* userEventCreateFx({
				...
				event: "transaction.open",
				isTerminal: false,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t4Open })));
			...
			yield* userEventCreateFx({
				...
				isTerminal: true,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t4Expired })));
			...
			yield* userEventCreateFx({
				...
				event: "transaction.open",
				isTerminal: false,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t5Open })));
			...
			yield* userEventCreateFx({
				...
				event: "transaction.message",
				isTerminal: false,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t5Msg })));
			...
			yield* userEventCreateFx({
				...
				event: "transaction.success",
				isTerminal: true,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t5Success })));
			...
			yield* userEventCreateFx({
				...
				event: "transaction.create",
				isTerminal: false,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t6Create })));
			...
			yield* userEventCreateFx({
				...
				event: "transaction.open",
				isTerminal: false,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t6Open })));
			...
			yield* userEventCreateFx({
				...
				event: "transaction.message",
				isTerminal: false,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t6Msg })));
			...
			yield* userEventCreateFx({
				...
				event: "transaction.closed",
				isTerminal: true,
			}).pipe(Effect.provide(DateContextLayer({ now: () => t6Close })));
