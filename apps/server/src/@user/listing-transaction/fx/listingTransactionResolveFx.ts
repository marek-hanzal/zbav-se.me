...
export namespace listingTransactionResolveFx {
...
export const listingTransactionResolveFx = ({
	listingTransactionId,
	message = "You are not allowed to access this listing transaction",
}: listingTransactionResolveFx.Props) => {
	return Effect.gen(function* () {
		...
		if (!transaction.status) {
			return yield* new RuntimeError({
				message: "Listing transaction status is missing",
			});
		}
...
export type listingTransactionResolveFx = ReturnType<typeof listingTransactionResolveFx>;
