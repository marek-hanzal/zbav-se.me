...
export const withFetchApiFx = Effect.fn("withFetchApiFx")(function* () {
	...
	userHono.openapi(
		createRoute({
			method: "post",
			path: "/inbox/fetch",
			description: "Returns one inbox entry based on provided query",
			operationId: "apiInboxFetch",
			request: {
				body: {
					content: {
						"application/json": {
		...
		async (c) => {
			...
			return Effect.gen(function* () {
				...
				withLoggingFx(axiomConfig, "apiInboxFetch", c.get("traceId")),
				withCatchFx({
					NotFoundErrorFx() {
