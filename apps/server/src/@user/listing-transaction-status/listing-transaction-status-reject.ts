...
import { listingTransactionStatusRejectFx } from "./fx/listingTransactionStatusRejectFx";
import { ListingTransactionStatusRejectSchema } from "./schema/ListingTransactionStatusRejectSchema";

export const withListingTransactionStatusRejectApi: Routes.Fn = ({ userHono }) => {
	userHono.openapi(
		createRoute({
			method: "post",
			path: "/listing-transaction/status/reject",
			description: "Reject a listing transaction. Requires access to the transaction.",
			operationId: "apiListingTransactionStatusReject",
			request: {
				body: {
					content: {
						"application/json": {
							schema: ListingTransactionStatusRejectSchema,
						},
					},
		...
		async (c) => {
			return Effect.gen(function* () {
				return c.json<ListingTransactionStatusSchema.Type, 200>(
					yield* listingTransactionStatusRejectFx(c.req.valid("json")),
					200,
				);
			}).pipe(
