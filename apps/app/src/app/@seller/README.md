...
- Seller listing-level transaction screen now lives in active scope at `@seller/transaction-listing/*`; its hero banner is split into a standalone component and seller transaction rows now show status-driven content plus client-side last-activity preview instead of repeated listing data.
