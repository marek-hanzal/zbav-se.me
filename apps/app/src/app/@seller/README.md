...
- Seller listing-level transaction screen now lives in active scope at `@seller/transaction-listing/*`; its hero banner and item stack now rely on the route-level pending page as the single suspense/loading gate, while local `ui/Item/*` pieces keep seller-local status label mapping, `StatusIcon.tsx`, and folder-local `Preview/*` for the final loaded UI.
