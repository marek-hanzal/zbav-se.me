...
- Seller transaction detail no longer archives unread buyer-message inbox rows on open; seller unread state now clears only after an explicit seller action (message send, accept/reject, dispute/resolve), so browsing the thread does not prematurely mark work as handled.
...
- Seller transaction-list sections now split server-side into `active`, `handled`, and `closed` by querying real child `transaction.status` state in SQL, instead of relying on client-side grouping heuristics or extra aggregate payload flags.
...
- Seller listing and draft list screens are flattening `Data.tsx` + `Pending.tsx` wrappers into parent `withFallback(...)` components:
  - page components own `Suspense` boundaries
  - list components suspend around item rendering instead of through dedicated wrapper files
