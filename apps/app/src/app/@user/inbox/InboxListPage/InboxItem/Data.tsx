...
export const Data: FC<Data.Props> = ({ _suspense, inboxId }) => {
	...
	return match(item)
		.with({ type: "seller-message" }, (item) => <InboxSellerMessageItem item={item} />)
		.with({ type: "buyer-message" }, (item) => <InboxBuyerMessageItem item={item} />)
		.with({ type: "transaction" }, (item) => <InboxTransactionItem item={item} />)
		.with({ type: "system" }, (item) => <InboxSystemItem item={item} />)
		.with({ type: "unknown" }, (item) => <InboxUnknownItem item={item} />)
		.with({ type: "thumb" }, (item) => <InboxThumbItem item={item} />)
		.with({ type: "favourite" }, (item) => <InboxFavouriteItem item={item} />)
		.with({ type: "unfavourite" }, (item) => <InboxUnfavouriteItem item={item} />)
		.exhaustive();
};
