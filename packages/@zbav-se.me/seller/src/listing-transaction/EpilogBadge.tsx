...
export const EpilogBadge: FC<EpilogBadge.Props> = ({ listingTransaction, ...props }) => {
	...
	return match(listingTransaction.status)
		...
		.with("rejected", () => {
			return (
				<Badge {...defaultProps}>
					<Tx label={"Seller rejected the transaction - seller (label)"} />
				</Badge>
			);
		})
		.with("expired", () => {
			return (
				<Badge {...defaultProps}>
					<Tx label={"This transaction has expired - seller (label)"} />
				</Badge>
			);
		})
		.with("success", () => {
			return (
				<Badge {...defaultProps}>
					<Tx
						label={"This transaction has been successfully completed - seller (label)"}
					/>
				</Badge>
			);
		})
		.with("closed", () => {
			return (
				<Badge {...defaultProps}>
					<Tx label={"This transaction has been closed - seller (label)"} />
				</Badge>
			);
		})
		.exhaustive();
