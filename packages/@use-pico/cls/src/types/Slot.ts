...
export namespace Slot {
	...
	export type Has<TContract extends Contract.Any> = TContract["slot"][number] extends never
		? false
		: true;

	export type With<TContract extends Contract.Any> = Raw<TContract> extends never ? false : true;
	...
	export type Required<TContract extends Contract.Any> =
		With<TContract> extends false ? Record<string, never> : SlotOf<TContract>;
	...
	export type Optional<TContract extends Contract.Any> =
		With<TContract> extends false ? Record<string, never> : Partial<SlotOf<TContract>>;
