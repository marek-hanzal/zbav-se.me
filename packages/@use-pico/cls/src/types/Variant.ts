...
export namespace Variant {
	 ...
	 */
	export type Has<TContract extends Contract.Any> = keyof TContract["variant"] extends never
		? false
		: true;
	 ...
	 */
	export type With<TContract extends Contract.Any> = keyof Raw<TContract> extends never
		? false
		: true;
	...
	export type Required<TContract extends Contract.Any> =
		With<TContract> extends false ? Record<string, never> : VariantOf<TContract>;

	export type Optional<TContract extends Contract.Any> =
		With<TContract> extends false ? Record<string, never> : Partial<VariantOf<TContract>>;
}
