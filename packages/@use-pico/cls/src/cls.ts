...
type Predicate<TContract extends Contract.Any> = (variant: Variant.VariantOf<TContract>) => boolean;
...
function compilePredicate<TContract extends Contract.Any>(
	...
	return function pred(variant) {
		...
		const variantAny: Record<string, unknown> = variant as unknown as Record<string, unknown>;
...
function buildLayers<TContract extends Contract.Any>(
	...
	let currentDefinition: Definition.Type<TContract> | Definition.Type<Contract.Any> | undefined =
		definition;
	...
	while (currentContract && currentDefinition) {
		...
		currentDefinition = currentContract ? (currentContract as any)["~definition"] : undefined;
	}
...
function indexRulesBySlot<TContract extends Contract.Any>(
	...
	const rules: Rule.Type<Contract.Any>[] = layers.flatMap(({ definition }) => {
	...
	return pairs.reduce<Record<string, CompiledRule<TContract>[]>>((accumulator, pair) => {
...
function buildTokenTable(
	...
	return layers.reduce<Record<string, What.Any<Contract.Any>>>((accumulator, item) => {
		const layerTokensEntries = Object.entries(item.definition.token ?? {}).filter(
			([, value]) => {
				return value !== undefined;
			},
		);
		const layerTokens = Object.fromEntries(layerTokensEntries) as Record<
			string,
...
function createResolver(baseTokenTable: Record<string, What.Any<Contract.Any>>) {
	const baseResolvedTokenCache: Record<string, ClassName[]> = Object.create(null);

	/**
	 * Push class names into the accumulator, supporting string | string[] | nested arrays.
	 * Order is preserved; falsy entries are ignored.
	 */
	function pushClassNames(accumulator: ClassName[], classInput: unknown): void {
...
>(contract: TContract, definition: Definition.Type<TContract>): Cls.Type<TContract> {
	contract["~definition"] = definition;

	// Precompile layers, slots, rules and base token table
	const { slotKeys, rulesBySlot, rulesByToken, tokensProto } = compileContract(
		contract,
		definition,
	);
	...
	return {
		create(...tweak) {
			...
			const tokenTable: Record<string, What.Any<Contract.Any>> = $tweak?.token
			...
			const slotFunctionCache: Record<TSlot[number], CoolSlot.Fn<TContract>> = Object.create(
				null,
			);
			...
			function computeKeyFromLocal<TContractLocal extends Contract.Any>(
				...
				const localSlotTable = local.slot as unknown as Record<string, unknown> | undefined;
			...
			const handler: ProxyHandler<Record<TSlot[number], CoolSlot.Fn<TContract>>> = {
					...
					const slotFunction: CoolSlot.Fn<TContract> = (...local) => {
						...
						const tokenRulesOverlayEntries = Object.entries(rulesByToken)
							.map(([tokenKey, compiledRules]) => {
								...
								const accumulated = compiledRules.slice(startIndex).reduce<{
									classes: ClassName[];
									tokens: string[];
								}>(
									(acc, rule, idx) => {
										...
										if (resolved.override) {
											acc.classes = resolved.classes.slice();
											acc.tokens = [];
										} else {
											acc.classes = acc.classes.concat(resolved.classes);
										}
										if ((rule.what as any).token) {
											acc.tokens = acc.tokens.concat(
												((rule.what as any).token as any[]).filter(
													Boolean,
												) as string[],
											);
										}
								...
								const composed: What.Any<Contract.Any> = {} as any;
								if (accumulated.classes.length > 0) {
									(composed as any).class = accumulated.classes;
								}
								if (accumulated.tokens.length > 0) {
									(composed as any).token = accumulated.tokens;
								}
								return [
									tokenKey,
									composed,
						...
						> = tokenRulesOverlayEntries.length > 0
							? Object.assign(
									...
									Object.fromEntries(tokenRulesOverlayEntries),
								)
							: Object.create(tokensProto);
						...
						let localResolvedCache: Record<string, ClassName[]> | undefined;
						if ($local?.token && Object.keys($local.token).length > 0) {
						...
						const localSlotWhat = $local?.slot
							? ($local.slot as Record<string, What.AnyOverride<Contract.Any>>)[
									slotName
								]
							: undefined;
						const configSlotWhat = $tweak?.slot
							? ($tweak.slot as Record<string, What.AnyOverride<Contract.Any>>)[
									slotName
								]
							: undefined;
			...
			return {
				slots: new Proxy({} as Record<TSlot[number], CoolSlot.Fn<TContract>>, handler),
				variant,
			};
		},
