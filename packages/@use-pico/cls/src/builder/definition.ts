...
>(state: DefinitionBuilder.State<TContract, TUse>): DefinitionBuilder.Builder<TContract, TState> {
	return {
		...
		tokens: {
			...
			>(key: TKey, value: TValue, token: Token.Optional<TContract>, override = false) {
				return builder({
					...state,
					rules: [
						...state.rules,
		...
		rule(match: Variant.Optional<TContract>, slot: Slot.Optional<TContract>, override = false) {
			return builder({
				...state,
				rules: [
					...state.rules,
		...
		switch(key: any, whenTrue: Slot.Optional<TContract>, whenFalse: Slot.Optional<TContract>) {
			return builder({
				...state,
				rules: [
					...state.rules,
		...
		match<
			const TKey extends keyof Variant.VariantOf<TContract>,
			const TValue extends Variant.VariantOf<TContract>[TKey],
		>(key: TKey, value: TValue, slot: Slot.Optional<TContract>, override = false) {
			return builder({
				...state,
				rules: [
					...state.rules,
