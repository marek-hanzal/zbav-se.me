...
describe("builder-03-complex/three-level-four-slots-mixed-overrides-and-overlays", () => {
	it("handles mixed per-slot overrides and dual overlays across four slots", () => {
		...
		const grandchildButton = definition(contract(childButton.contract).build())
			.root({
				root: {
					class: [
						"root-grandchild",
		...
		expect(created.slots.root()).toBe("CONFIG-PRIMARY root-base root-child root-grandchild");
		expect(created.slots.icon()).toBe("CONFIG-PRIMARY icon-base icon-child icon-grandchild");
		expect(created.slots.label()).toBe(
			"label-base label-child label-grandchild LABEL-OVERRIDE",
		);
		expect(created.slots.badge()).toBe("badge-base badge-child badge-grandchild USER-BADGE");
	});
});
