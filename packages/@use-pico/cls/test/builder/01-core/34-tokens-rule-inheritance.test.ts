...
describe("builder/tokens-rule-inheritance", () => {
	it("token rules work with contract inheritance", () => {
		...
		expect(slots.root()).toBe("primary-token secondary-token base accent-token");
		...
		).toBe("primary-base-rule secondary-disabled-rule base accent-primary-disabled-rule");
	});
});
