...
describe("react/01-core/append-order-across-two-slots-I-P-U", () => {
	it("appends in order internal, then provider, then user across root and icon", () => {
		...
		expect(result.current.slots.root()).toBe("base user-root-append internal-root-append");
		expect(result.current.slots.icon()).toBe("icon-base user-icon-append internal-icon-append");
	});
});
