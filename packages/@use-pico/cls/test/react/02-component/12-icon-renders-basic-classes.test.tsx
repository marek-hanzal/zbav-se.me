...
const SimpleIcon: FC<SimpleIconProps> = ({ icon, size, disabled, cls = SimpleIconCls, tweak }) => {
...
describe("react/02-component/icon-renders-basic-classes", () => {
	it("renders with Icon-root and provided icon class", () => {
		...
		const root = container.querySelector('[data-ui="Icon-root"]') as HTMLElement;
		expect(root).toBeTruthy();
		expect(root.className).toBe("Icon-root w-8 h-8 icon-[rivet-icons--check]");
	});
});
