...
describe("react/02-component/fade-component-token-override", () => {
	it("uses default component tokens when no provider or override", () => {
		...
		const topElement = container.querySelector('[data-ui="Fade-top"]') as HTMLElement;
		const bottomElement = container.querySelector('[data-ui="Fade-bottom"]') as HTMLElement;
	...
	it("component token override with empty array clears token", () => {
		...
		expect(topElement?.className).toBe("theme-to-color fade-top-base absolute top-0");
		expect(bottomElement?.className).toBe("theme-to-color fade-bottom-base absolute bottom-0");
	});
	...
	it("mixed token tweaks and overrides work correctly", () => {
		...
		// User token overrides should take precedence over component variant rules
		expect(topElement?.className).toBe("override-from tweak-via fade-top-base absolute top-0");
		expect(bottomElement?.className).toBe(
	...
	it("simple token tweaks with empty arrays work", () => {
		...
		expect(bottomElement?.className).toBe("fade-bottom-base absolute bottom-0");
	});
