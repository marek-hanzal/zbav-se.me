...
describe("cls/complex/five-slot-stress-many-rules-order-verified", () => {
	it("5 slots with many rules; verify deterministic order across slots", () => {
		...
		expect(slots.header()).toBe("h-base h-sm h-dark h-on h-hover h-sm-dark");
		expect(slots.content()).toBe("c-base c-sm c-dark c-on c-hover c-sm-dark");
		expect(slots.footer()).toBe("f-base f-sm f-dark f-on f-hover f-sm-dark");
