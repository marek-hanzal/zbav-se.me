import type { Migration } from "kysely/migration";

export const UserResourceBundleLimitStripeMigration: Migration = {
	async up(db) {
		await db.schema
			.createTable("user_resource_bundle_limit_stripe")
			.addColumn("id", "text", (col) => col.primaryKey().notNull())
			.addColumn("userResourceBundleLimitId", "text", (col) => col.notNull())
			.addColumn("key", "text", (col) => col.notNull())
			.addColumn("createdAt", "timestamptz", (col) => col.notNull())
			.addForeignKeyConstraint(
				"user_resource_bundle_limit_stripe_[userResourceBundleLimitId]_fk",
				[
					"userResourceBundleLimitId",
				],
				"user_resource_bundle_limit",
				[
					"id",
				],
				(c) => c.onDelete("cascade"),
			)
			.addUniqueConstraint(
				"user_resource_bundle_limit_stripe_[userResourceBundleLimitId-key]_unique_idx",
				[
					"userResourceBundleLimitId",
					"key",
				],
			)
			.execute();

		await db.schema
			.createIndex("user_resource_bundle_limit_stripe_[key]_idx")
			.on("user_resource_bundle_limit_stripe")
			.column("key")
			.execute();
	},
};
