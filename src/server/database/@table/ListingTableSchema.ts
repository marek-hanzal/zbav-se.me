...
const DraftSchema = z
	.looseObject({
		...BaseSchema.shape,
		status: z.literal("draft" satisfies ListingStatusEnumSchema.Type),
		//
		title: TitleSchema.nullish(),
		//
		locationId: z.string().min(1).nullish(),
		withLocation: z.unknown().nullish().meta({
...
const LiveSchema = z
	.looseObject({
		...BaseSchema.shape,
		status: z.literal("live" satisfies ListingStatusEnumSchema.Type),
		//
...
export const FooSchema = z.discriminatedUnion("type", [
	DraftSchema,
	LiveSchema,
]);
