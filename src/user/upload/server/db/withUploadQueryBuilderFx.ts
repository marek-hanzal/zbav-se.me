...
import type { withUploadSourceSelectFx } from "~/user/upload/server/db/withUploadSourceSelectFx";
...
>({ select, where }: withUploadQueryBuilderFx.Props<TSelect>) {
	...
	if (where.userId) {
		query = query.where("u.userId", "=", where.userId) as TSelect;
	}

	if (where.access) {
		query = query.where("u.access", "=", where.access) as TSelect;
	}
