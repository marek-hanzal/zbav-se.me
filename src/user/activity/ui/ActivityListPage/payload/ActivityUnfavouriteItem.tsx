...
import type { UnfavouriteSchema } from "~/server/database/@table/ActivityTableSchema/UnfavouriteSchema";
import { withActivityQuery } from "~/user/activity/query/withActivityQuery";

export namespace ActivityUnfavouriteItem {
	export interface Props {
		item: UnfavouriteSchema.Type;
	}
}

export const ActivityUnfavouriteItem: FC<ActivityUnfavouriteItem.Props> = ({ item }) => {
		...
		<ListItem
			...
			title={
				<Tx
					...
					data-ui-tone={item.archivedAt ? "neutral" : "primary"}
					data-ui-theme="light"
