...
- location: Share a location/address. Use only with a concrete locationId; if the user gives a human address, normalize it with the location tool first.
- personal: Share handover/contact details. Use only when the user explicitly provided name, phone, email, and location. Normalize the location to locationId first.
- package: Share shipping/tracking details. Seller-only. Requires a valid tracking link. Use number when known, otherwise set number to null.
...
- In dispute transactions, buyer and seller may create text, gallery, location, and personal entries; only seller may create package entries.
...
This writes to the transaction timeline and notifies the other participant, so do not call it for drafts, previews, suggestions, or when required fields are uncertain.
	`.trim(),
