...
description: Create or modify apps/server endpoints using the repo's Effect + Hono + Zod + Kysely conventions, including with*ApiFx registration and withCatchFx error mapping.
---
...
- `withCatchFx`
- `Effect.runPromise`
4. Validate payload/output with `zodGuardFx` + Zod schemas.
5. Register endpoint from feature `with*ApiFx.ts` and mount domain route in top-level domain `with*ApiFx.ts` if needed.
...
7. Run checks relevant to server changes.
...
- Errors are wired in consistent style.
