...
describe("subscriptionSyncFx", () => {
	...
	liveIt(
		"uses checkout session resource bundle fallback when subscription metadata is missing",
		async () => {
			if (!stripeSecret) {
				throw new Error("SERVER_STRIPE_SECRET is required");
			}

			const database = await testabase("stripe-subscription-sync-live-session-fallback");
			const stripe = new Stripe(stripeSecret);
			const customerIds: string[] = [];
			const subscriptionIds: string[] = [];
			...
			try {
				return await Effect.gen(function* () {
					...
					const customer = yield* Effect.promise(() => {
						return stripe.customers.create({
							email: `stripe-sync-fallback-${buyer.id}@example.test`,
						});
					});
					customerIds.push(customer.id);
