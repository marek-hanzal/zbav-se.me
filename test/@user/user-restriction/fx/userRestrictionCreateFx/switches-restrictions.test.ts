...
describe("userRestrictionCreateFx", () => {
	it("creates a single live restriction for immediate switches", async () => {
		const database = await testabase("user-restriction-create-immediate");
		...
		return Effect.gen(function* () {
			...
			expect(waitingRestrictions).toHaveLength(0);
	...
	it("expires current and waiting restrictions when an immediate switch is created", async () => {
		const database = await testabase("user-restriction-create-immediate-replaces-all");
		...
		return Effect.gen(function* () {
			...
			const waitingRestrictions = yield* fetchWaitingRestrictions(user.id);
			const storedRestrictions = yield* fetchStoredRestrictions(database, user.id);
			const expiredRestrictions = storedRestrictions.filter(
